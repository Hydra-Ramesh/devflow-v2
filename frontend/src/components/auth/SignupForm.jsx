...
export default function SignupForm() {
  ...
  const { signUpWithEmail, signInWithProvider } = useAuthStore();
  ...
  const onSubmit = async (data) => {
    try {
      setLoading(true);
      setAuthError(null);
      await signUpWithEmail(data.email, data.password, { full_name: data.fullName });
      navigate('/');
    } catch (err) {
  ...
  return (
    ...
    >
      <div className="text-left space-y-2">
        ...
        <h2 className="text-4xl font-bold tracking-tight text-zinc-900 dark:text-white">Create Account</h2>
        <p className="text-zinc-500 dark:text-zinc-400 text-sm">Join the DevFlow community today</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="space-y-2">
          <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 ml-1">Full Name</label>
          <input
            type="text"
      ...
      <p className="text-center text-sm text-zinc-500 dark:text-zinc-400 mt-8">
        ...
        <Link to="/login" className="font-semibold text-zinc-900 dark:text-white hover:underline decoration-2 underline-offset-4">
          Sign in
        </Link>
      </p>
    </motion.div>
