...
public class KafkaConfig {
    ...
    public void publishEvent(KafkaTemplate<String, Object> template, String topic, String key, Map<String, Object> payload) {
        Map<String, Object> eventMessage = new HashMap<>();
        eventMessage.put("eventId", UUID.randomUUID().toString());
        eventMessage.put("timestamp", new Date().toString());
        eventMessage.put("eventType", topic);
        eventMessage.put("payload", payload);

        template.send(topic, key != null ? key : UUID.randomUUID().toString(), eventMessage);
    }
}
